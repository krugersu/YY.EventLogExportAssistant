    ...
    public class EventLogExportMasterTests
    {
        ...
        public EventLogExportMasterTests()
        {
            ...
            _settings = new CommonTestSettings(
                configFilePath,
                new EventLogSQLServerActions());

            _optionsBuilder = new DbContextOptionsBuilder<EventLogContext>();
            _optionsBuilder.UseSqlServer(_settings.ConnectionString);
        ...
        {
            ExportToSQLServer(_settings.SettingsLGF);
        }
        [Fact]
        public void ExportFormatLGDToSQLServerTest()
        {
            ExportToSQLServer(_settings.SettingsLGD);
        }
        ...
        private void ExportToSQLServer(EventLogExportSettings eventLogSettings)
        {
            EventLogOnSQLServer target = new EventLogOnSQLServer(_optionsBuilder.Options, eventLogSettings.Portion);
        ...
        private string GetConfigFile()
        {
            ...
            if (!File.Exists(configFilePath))
            {
                ...
                try
                {
                    ...
                    using (EventLogContext context = EventLogContext.Create(_optionsBuilder.Options, new EventLogSQLServerActions()))
                        context.Database.EnsureDeleted();
                }
                catch
                {
